...
const onRejectedApiResponse = (error: AxiosError<ApiResponse>) => {
  if (error.response) {
    ...
    if (error.response.status === 401) {
      const skipAuthRedirectHeader = error.config?.headers?.['x-skip-auth-redirect'];
      const shouldSkipRedirect = skipAuthRedirectHeader === 'true' || skipAuthRedirectHeader === true;
